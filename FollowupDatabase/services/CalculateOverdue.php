<?php
    ...
    //Every Monday send an email to each BHS containing an order list of his or her active events
    if(date("l") == "Monday")
    {
	    //For each user, find all active clients
	    foreach ($users as $user)
	    {
	    	if($user->initials == "ZZ")
	    		continue;
		    ...
		    $eventsByDate = array();
		    $c = new ClientService();	
		    $clients = $c->searchClients("","'Active'",$user->initials,"");
		    
		    //Then for each client, find all active events
		    foreach ($clients as $client)
		    {
		    	$e = new EventService();
		    	...
		    	{
		    		$event->wits_id = $client->wits_id;
		    		if($event->status == "Active")
		    		{
		    			$eventsByDate[strval($event->targetdate)] = $event;
		    			
		    		}
		    	}
		    }
		    
		    //Output all active events for this user
		    //Send an email containing that output
		    ksort($eventsByDate);
		    $output = "<div>These are all of your currently Active followup events, ordered by date.<br /><br />";
		    foreach ($eventsByDate as $event)
		    {
		    	$output .= date("D m/d/y", $event->targetdate);
		    	$output .= " - " . $event->wits_id;
		    	$output .= " - " . $event->type;
		    	$output .= "<br />";
		    }
		    if(count($eventsByDate) == 0)
		    	$output .= "You currently have no Active events.";
		    $output .= "</div>";
		    
		    $mail = new Zend_Mail();
			$mail->setBodyHtml($output);
			$mail->setFrom('EXAMPLE@EXAMPLE.com', 'Followup-Up Support System');
			$mail->addTo($user->email, 'BHS');
			$mail->setSubject('Follow-Up Support System: Event Listing');
			$mail->send($transport);
    	}
    }
